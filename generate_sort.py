...
def generate_sequence(length,max_num, f_source, f_target):
    ...
    sort = matrix.flatten() #flatten to numpy array
    num = str(random.randint(0,max_num))
    matrix = matrix.flatten()
    sort = np.array(sort.tolist()) #convert to Python list
    sort = sorted(sort) #sort the Python list
    ...
    str_transpose = np.array2string(sort)
